...
<p>Recall \(I=[0,1]\). The Cartesian product \(A\times B=\{(a,b):a\in A,\ b\in B\}\) is the set of pairs with one coordinate in each set. Thus \(I^2=I\times I\) is the unit square, and \(C:I^2\to I\) means that \(C\) assigns each point of the square a value in \([0,1]\).</p>
...
<article class="exercise"><header class="exercise-head"><div><strong>Exercise 2.6</strong><span>Convex mixtures</span></div><button class="answer-button" type="button">Show answer</button></header><div class="exercise-body"><p>Show that \(C_q=(1-q)C_0+qC_1\) is a copula for \(0\le q\le1\).</p></div><div class="answer-panel"><div class="answer-inner"><p>Boundary values are preserved by linearity. Write \(V_{C_i}\) for the four-corner volume of \(C_i\) on the rectangle being checked, for \(i=0,1\). The volume is \((1-q)V_{C_0}+qV_{C_1}\ge0\). Hence \(C_q\) satisfies every copula condition.</p></div></div></article>
</div>
