...
<p>The [[Gaussian copula]] is one member of an elliptical family. A useful construction starts with a direction \(S\) uniformly distributed on the unit sphere \(\{s\in\mathbb R^d:\sum_{j=1}^d s_j^2=1\}\), where \(\mathbb R^d\) is the set of real \(d\)-coordinate vectors, an independent nonnegative radial variable \(Q\), a vector \(\mu\), and a full-rank matrix \(B\). Set \(X=\mu+QBS\). The direction supplies a symmetric shape; the radial variable determines how far a draw lies from its center. The matrix \(\Sigma=BB^\mathsf T\) describes that shape; the superscript \(\mathsf T\) means transpose. This is the full-rank version of the spherical representation in Definition 4.5. <a class="course-citation" href="#ref-1" aria-label="Reference 1">[1]</a></p>
...
<p>To simulate in dimension \(d\), factor \(R=LL^\mathsf{T}\), where \(L\) is lower triangular. Draw \(Y\) with independent standard normal components and set \(Z=LY\). Here \(E[Z]\) is the vector of coordinate means, and \(\operatorname{Cov}(Z)\) is the matrix with entries \(\operatorname{Cov}(Z_i,Z_j)\). Then \(E[Z]=0\) and</p>
...
<li id="ref-1">Jan-Frederik Mai and Matthias Scherer, <em>Simulating Copulas: Stochastic Models, Sampling Algorithms, and Applications</em>, 2nd ed. (2017), §4.4, Definition 4.5, Remark 4.3, and Lemma 4.6(3), printed pp. 174–175 (PDF pp. 193–194): elliptical representation, shape normalization, and Kendall's coefficient.</li>
<li id="ref-2">Jan-Frederik Mai and Matthias Scherer, <em>Financial Engineering with Copulas Explained</em> (2014), §4.1, equations (4.1)–(4.2), printed pp. 49–50 (PDF pp. 66–67): Gaussian construction; Example 2.5.1, printed pp. 30–31 (PDF pp. 47–48): copula derivatives.</li>
<li id="ref-3">Roger B. Nelsen, <em>An Introduction to Copulas</em> (2006), §2.10, printed pp. 42–49 (PDF pp. 53–60): Gaussian copula construction.</li>
</ol>
