...
<section class="intro-strip" id="question"><h2>Why sample a posterior we can integrate?</h2><p>Lesson 23 gives an exact posterior for the same two observations, \((1/4,1/4)\) and \((3/4,3/4)\). That exact answer gives us a check for an implementation before we apply MCMC to a posterior without a convenient normalizing constant. The book applies Metropolis to copula estimation in Example 7.3.6. Here we apply that approach to the smaller model already derived in the course. <a class="course-citation" href="#ref-1" aria-label="Reference 1">[1]</a></p></section>
