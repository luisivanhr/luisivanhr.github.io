...
date: 2026-01-01
---
<section class="intro-strip" id="conditional"><h2>conditional sampling</h2><p>For a bivariate copula, the right-continuous version of the partial derivative \(\partial_2C(u,v)\) is a conditional distribution function of \(U_1\) given \(U_2=v\) for almost every \(v\). Since a simulated \(U_2\) avoids any exceptional null set with probability one, this gives a general algorithm.</p></section>
<div class="math-block theorem"><span class="block-label">Theorem 10.1 <span>Conditional algorithm</span></span><p>Draw \(U_2\sim U[0,1]\), draw \(V\sim U[0,1]\) independently, set \(F_{1|2}(u)=\partial_2C(u,U_2)\) using that valid conditional version, and return \((F_{1|2}^{[-1]}(V),U_2)\). The result has copula \(C\).</p></div>
...
<h2>Source note</h2><p>Jan-Frederik Mai and Matthias Scherer, <em>Simulating Copulas: Stochastic Models, Sampling Algorithms, and Applications</em>, second edition (2017), §1.1.3, Algorithm 1.2, printed pp. 22–24 (PDF pp. 41–43), for the conditional derivative, its right-continuous version, generalized inverse, and bivariate sampling; §2.2, Algorithm 2.1, printed pp. 61–62 (PDF pp. 80–81), for the shared frailty construction of extendible Archimedean copulas. Roger B. Nelsen, <em>An Introduction to Copulas</em> (2006), §2.9, equation (2.9.1) and the sampling steps, printed p. 41 (PDF p. 52), gives the equivalent conditional algorithm.</p>
